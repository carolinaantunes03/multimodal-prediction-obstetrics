...
def train_and_validate(model, train_loader, val_loader, optimizer, criterion, device,
                       best_model_path, epochs, patience):
    """
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            '''
            if epoch == 0:
                print("[DEBUG] Batch input to model:")
                print("  images:", batch["images"].shape)
                print("  image_valid_num:", batch["image_valid_num"])
                print("  labels:", batch["label"])
            '''
            # --- handle both multimodal and image-only models ---
            if isinstance(batch, dict) and "images" in batch:
                try:
                    logits = model(batch["images"], image_valid_num=batch.get("image_valid_num"))
                except TypeError:
                    logits = model(batch["images"])  # for encoders that don't take image_valid_num
            else:
                logits = model(batch)
...
def evaluate(multimodal_model, test_loader, device):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            if isinstance(batch, dict) and "images" in batch:
                try:
                    outputs = multimodal_model(batch["images"], image_valid_num=batch.get("image_valid_num"))
                except TypeError:
                    outputs = multimodal_model(batch["images"])
            else:
